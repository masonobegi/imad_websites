// Reads the already-processed images from public/photos and regenerates data files.
// Run this any time photos are added or photos.ts needs to be refreshed.
// No sharp/exifr needed — just reads filenames from disk.

const fs = require('fs')
const path = require('path')

const PHOTOS_DIR = 'c:/Users/mason/OneDrive/Desktop/Imad_website/photography/public/photos'
const OUT_TS    = 'c:/Users/mason/OneDrive/Desktop/Imad_website/photography/lib/photos.ts'
const OUT_JSON  = path.join(PHOTOS_DIR, 'data.json')
...
const DESCRIPTIONS = {
  ...
  'fire suns': 'During the devastating 2017 North Bay fires, smoke turned the Bay Area sun blood-red for days — an eerie, apocalyptic light unlike anything most residents had ever seen. This image was made in the middle of the afternoon.',
...
for (const [cat, meta] of Object.entries(CATEGORIES)) {
  ...
  if (!fs.existsSync(dir)) { allPhotos[cat] = []; continue }
  const files = fs.readdirSync(dir).filter(f => f.endsWith('.jpg')).sort()
  allPhotos[cat] = files.map(filename => {
...
// Write JSON (read at runtime by pages — no module cache issues)
fs.writeFileSync(OUT_JSON, JSON.stringify({ categories: CATEGORIES, photos: allPhotos }, null, 2))
console.log('✅ Wrote', OUT_JSON)

// Write photos.ts (used by PhotoModal and CartContext for type info)
const SIZES = [
...
const ts = `// Auto-generated by scripts/watermark/generate-data.js — do not edit
...
export const CATEGORIES: Record<string, { label: string; description: string }> = ${JSON.stringify(CATEGORIES, null, 2)}
