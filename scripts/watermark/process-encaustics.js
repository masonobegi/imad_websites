...
function parseFilename(raw) {
  ...
  // No word boundary — handles "Available20191207" (digit directly after Available)
  const available = /available/i.test(s)

  // Remove "- Available", "-Available", " Available" etc
  s = s.replace(/\s*[-–]\s*available/gi, '')
  s = s.replace(/available/gi, '')

  // Remove camera/date codes — IMG_ as a unit FIRST to prevent first regex eating the numeric part
  s = s.replace(/\s+IMG_[\d_]+/gi, '')                              // IMG_0056, IMG_20200612_095253_752
  s = s.replace(/\s+DSC\d+/gi, '')                                  // DSC04524
  s = s.replace(/\s+\d{4}-\d{2}-\d{2}\s+\d{2}\.\d{2}\.\d{2}/, '') // 2018-06-25 22.34.10
  s = s.replace(/[\s_]?(20\d{6}[_]\d{6})([_]\w+)*\s*$/i, '')      // 20231206_191336, _rtch, _full_frame

  // Extract size — x-separated ("6_ x 6_") or space-separated inch markers ("8_ 8_")
  let originalSize = null
  const sizeMatchX = s.match(/(\d+)[_.]?\s*[xX×]\s*(\d+)[_.]?/)
  if (sizeMatchX) {
    originalSize = `${sizeMatchX[1]}" × ${sizeMatchX[2]}"`
  } else {
    const sizeMatchSpace = s.match(/(\d+)[_]\s+(\d+)[_]/)
    if (sizeMatchSpace) {
      originalSize = `${sizeMatchSpace[1]}" × ${sizeMatchSpace[2]}"`
    }
  }

  // Remove size patterns (both forms)
  s = s.replace(/\d+[_.]?\s*[xX×]\s*\d+[_.]?/g, '')
  s = s.replace(/\d+[_]\s+\d+[_]/g, '')
  // Strip orphaned "digit_" inch markers left after size removal
